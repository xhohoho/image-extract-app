    ...
    <div style={{ background: t.bg, minHeight: "100vh", transition: "background 0.2s" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
          ...
          <span style={{ fontWeight: 700, fontSize: "0.95rem", color: t.text, letterSpacing: "-0.01em" }}>
            ImageExtract
          </span>
          <span style={{
            fontSize: "0.65rem", fontWeight: 600, padding: "2px 6px",
            background: dark ? "#1e1e3a" : "#ede9fe",
            color: t.accentText, borderRadius: 4, letterSpacing: "0.05em",
          }}>AI</span>
        </div>
        <button
        ...
        <div style={{
          ...
          <div style={{ padding: "1.5rem" }}>
            ...
            <div
              ...
              style={{
                border: `2px dashed ${dragging ? t.borderHover : t.border}`,
                borderRadius: 12,
                ...
                background: dragging
                  ? (dark ? "#1a1a2e" : "#ede9fe")
                  : (preview ? t.previewBg : (dark ? "#0d0d14" : "#f9fafb")),
                transition: "all 0.2s",
                overflow: "hidden",
              }}
            >
              ...
              {preview ? (
                ...
                <div style={{ textAlign: "center", padding: "2rem" }}>
                  ...
                  <p style={{ margin: "0 0 0.4rem", color: t.text, fontSize: "0.9rem", fontWeight: 500 }}>
                    Drop image here or{" "}
                    <span style={{ color: t.accent, textDecoration: "underline" }}>browse</span>
                  </p>
                  <p style={{ margin: 0, color: t.textMuted, fontSize: "0.8rem" }}>
            ...
            {file && (
              <div style={{
                ...
                background: dark ? "#0d0d14" : "#f3f4f6",
                borderRadius: 8, display: "flex", alignItems: "center", gap: "0.5rem",
              }}>
                <span style={{ fontSize: "0.8rem", color: t.textMuted }}>📎</span>
                <span style={{ fontSize: "0.82rem", color: t.textSubtle, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {file.name}
                </span>
                <span style={{ fontSize: "0.78rem", color: t.textMuted, flexShrink: 0 }}>
            ...
            {/* Status messages */}
            {appStatus === "uploading" && (
              <StatusBar color={t.accentText} bg={dark ? "#1a1a2e" : "#ede9fe"} border={dark ? "#2a2a4a" : "#c7d2fe"}>
                Uploading image to secure storage...
              </StatusBar>
            )}
            {appStatus === "extracting" && (
              <StatusBar color={t.accentText} bg={dark ? "#1a1a2e" : "#ede9fe"} border={dark ? "#2a2a4a" : "#c7d2fe"}>
                Gemini AI is reading your image...
              </StatusBar>
            )}
            {appStatus === "error" && (
              <StatusBar color={t.errorText} bg={t.errorBg} border={t.errorBorder}>
                {errorMsg}
              </StatusBar>
            )}
          </div>
...
function StatusBar({ children, color, bg, border }: {
